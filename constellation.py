...
def main():
    ...
    pos, vel, active, colors = init_particles(num_particles)
    ...
    while running:
        ...
        if len(targets) > 0 and (not scanning or np.any(active)):
            ...
            if len(active_pos) > 0:
                ...
                pos[active] = active_pos
                vel[active] = active_vel
                
        # Smoothly lerp towards gestural target colors
        colors = colors * 0.90 + np.array(target_color) * 0.10
